...
export default {
	...
	props: {
        ...
        typ_dokumentu: {
            type: "string",
            label: "Typ dokumentu",
            description: "Wybierz typ dokumentu, który chcesz utworzyć. Każdy typ ma predefiniowane opcje, które możesz zmienić poniżej.",
            options: [
                "🎙️ Nagranie",
                "📓 Dziennik"
            ],
		...
		opcje_podsumowania: {
			...
			description: `Wybierz opcje do uwzględnienia w Twoim podsumowaniu. Możesz wybrać wiele opcji.`,
			options: [
				"Podsumowanie",
				"Główne punkty",
			...
			default: this.typ_dokumentu === "🎙️ Nagranie" 
                ? ["Podsumowanie", "Główne punkty", "Elementy do wykonania", "Pytania uzupełniające"] 
                : ["Ogólny opis dnia", "Kluczowe wydarzenia", "Osiągnięcia", "Wyzwania", "Wnioski", "Plan działania", "Rozwój osobisty", "Refleksja", "Ocena dnia (1-100)"],
			optional: false,
		},
		opcje_meta: {
			type: "string[]",
	...
	async additionalProps() {
		// Dostosuj opcje podsumowania w zależności od wybranego typu dokumentu
		if (this.typ_dokumentu) {
			// Tu możemy dodać logikę dostosowywania opcji
		}
    ...
    async chunkFileAndTranscribe({ file }, openai) {
		...
		createSystemPrompt(
			...
			// Bazowy prompt jest taki sam dla obu typów dokumentów
			...
			// Dodawanie odpowiednich sekcji w zależności od typu dokumentu
			if (this.typ_dokumentu === "🎙️ Nagranie") {
				// Standardowe opcje dla nagrania
			...
			else if (this.typ_dokumentu === "📓 Dziennik") {
            ...
            // Dodawanie przykładów dla typu "🎙️ Nagranie"
            if (this.typ_dokumentu === "🎙️ Nagranie") {
            ...
            // Dodawanie przykładów dla typu "📓 Dziennik"
            else if (this.typ_dokumentu === "📓 Dziennik") {
		...
		async formatChat(summaryArray) {
			...
			let chatResponse = resultsArray.reduce(
				(acc, curr) => {
                    ...
                    // Sprawdzamy, jaki typ dokumentu został wybrany i odpowiednio agregujemy dane
                    if (this.typ_dokumentu === "🎙️ Nagranie") {
                        // Agregacja dla typu "Nagranie"
                        ...
                        // Wybieramy najwyższą ocenę dnia z wszystkich fragmentów
                    ...
                    // Agregacja własnych poleceń (wspólna dla obu typów)
            ...
            // Przygotowanie finalnej odpowiedzi w zależności od typu dokumentu
            let finalChatResponse;
            
            if (this.typ_dokumentu === "🎙️ Nagranie") {
                // Przygotowanie odpowiedzi dla typu "Nagranie"
                finalChatResponse = {
                    ...
                    ...(this.opcje_podsumowania.includes("AI rekomendacje") && {
                        ai_recommendations: chatResponse.ai_recommendations.flat(),
                    }),
                    ...(this.opcje_podsumowania.includes("Źródła do przejrzenia") && {
                        resources_to_check: chatResponse.resources_to_check.flat(),
                    }),
                    tokens: arraySum(chatResponse.usageArray),
                };
            } else if (this.typ_dokumentu === "📓 Dziennik") {
                // Przygotowanie odpowiedzi dla typu "Dziennik"
                finalChatResponse = {
                    title: chatResponse.title || "Wpis dziennika",
                    day_overview: chatResponse.day_overview.join(" ") || "Brak opisu dnia",
                    key_events: chatResponse.key_events.flat().length > 0 ? chatResponse.key_events.flat() : ["Brak kluczowych wydarzeń"],
                    achievements: chatResponse.achievements.flat().length > 0 ? chatResponse.achievements.flat() : ["Brak osiągnięć"],
                    challenges: chatResponse.challenges.flat().length > 0 ? chatResponse.challenges.flat() : ["Brak wyzwań"],
                    insights: chatResponse.insights.flat().length > 0 ? chatResponse.insights.flat() : ["Brak wniosków"],
                    action_plan: chatResponse.action_plan.flat().length > 0 ? chatResponse.action_plan.flat() : ["Brak planu działania"],
            ...
            // Dodaj własne polecenia do ostatecznej odpowiedzi, jeśli istnieją
            if (this.wlasne_polecenia_ai && chatResponse.custom_instructions.flat().length > 0) {
                finalChatResponse.custom_instructions = chatResponse.custom_instructions.flat();
            }
		...
		async createNotionPage(
			...
			const data = {
				...
				properties: {
					...
					...(this.wlasciwoscTagu && {
						[this.wlasciwoscTagu]: {
							select: { name: this.wartoscTagu || this.typ_dokumentu },
						},
					}),
			...
			// Przygotowanie sekcji w zależności od typu dokumentu
			if (this.typ_dokumentu === "🎙️ Nagranie") {
				...
				// Przygotowanie podsumowania dla dziennika
				if (meta.day_overview) {
					responseHolder.summary_header = "Ogólny opis dnia";
					const summaryHolder = [];
					const summaryBlockMaxLength = 80;
					
					// Traktujemy day_overview jako podsumowanie dla widoku Notion
					const overview = typeof meta.day_overview === 'string' ? [meta.day_overview] : meta.day_overview;
					
					for (let i = 0; i < overview.length; i += summaryBlockMaxLength) {
						summaryHolder.push(overview.slice(i, i + summaryBlockMaxLength));
					}
					responseHolder.summary = summaryHolder;
				}
			}

			// Przygotowanie nagłówka transkrypcji (wspólne dla obu typów)
			...
			// Przygotowanie transkrypcji (wspólne dla obu typów)
			...
			// Dodanie odpowiednich sekcji w zależności od typu dokumentu
			if (this.typ_dokumentu === "🎙️ Nagranie") {
				// Sekcje dla typu "Nagranie"
				const sections = [
					{
					...
					{
						arr: meta.ai_recommendations,
						header: "AI rekomendacje",
						itemType: "bulleted_list_item",
					},
					{
						arr: meta.resources_to_check,
						header: "Źródła do przejrzenia",
						itemType: "bulleted_list_item",
					},
				];

				// Dodanie niestandardowych poleceń, jeśli istnieją
				if (meta.custom_instructions && meta.custom_instructions.length > 0) {
					sections.push({
						arr: meta.custom_instructions,
						header: this.wlasne_polecenia_ai,
						itemType: "bulleted_list_item",
					});
				}

				// Dodanie wszystkich wybranych sekcji
				...
				// Sekcje dla typu "Dziennik"
				const sections = [
					{
						arr: meta.key_events,
						header: "Kluczowe wydarzenia",
						itemType: "bulleted_list_item",
					},
					{
						arr: meta.achievements,
						header: "Osiągnięcia",
						itemType: "bulleted_list_item",
					},
					{
					...
					{
						arr: meta.action_plan,
						header: "Plan działania",
						itemType: "to_do",
					},
					{
						arr: [meta.personal_growth],
						header: "Rozwój osobisty",
						itemType: "bulleted_list_item",
					},
					{
						arr: [meta.reflection],
					...
					{
						arr: [meta.day_rating ? `Ocena dnia: ${meta.day_rating}/100` : "Ocena dnia: nie określono"],
						header: "Ocena dnia",
						itemType: "bulleted_list_item",
					},
					{
	...
	async run({ steps, $ }) {
		...
		const logSettings = {
			"Typ dokumentu": this.typ_dokumentu,
			...
			"Dodać plik": this.dodac_plik ? "Tak" : "Nie",
			"Plan Notion": this.plan_notion || "Nie określono",
